...
async def shorten_url(url: str, request: Request):
    ...
    logger.info(
        "Short URL created | env=%s | request_id=%s | short_id=%s",
        ENV,
...
async def redirect(short_id: str, request: Request):
    logger.info(
        "Redirect request | env=%s | request_id=%s | short_id=%s",
        ENV,
        request.state.request_id,
        short_id
    )
    ...
    return {
        "message": f"Redirecting {short_id} (mock)"
    }
